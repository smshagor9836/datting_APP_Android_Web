...
namespace QuickDate.Activities.Chat
{
    ...
    public class MessagesBoxActivity : AppCompatActivity, IOnClickListenerSelectedMessages, MaterialDialog.IListCallback
    {
        ...
        private void ImageButtonOnClick(object sender, EventArgs e)
        {
            try
            {
                if (CanDoChat())
                {
                    ...
                    {
                        GetString(Resource.String.Lbl_ImageGallery),GetString(Resource.String.Lbl_TakeImageFromCamera)
                    };
                    ...
                    dialogList.PositiveText(GetText(Resource.String.Lbl_Close)).OnPositive(new MyAccountActivity());
        ...
        protected override async void OnActivityResult(int requestCode, Result resultCode, Intent data)
        {
            try
            {
                ...
                if (requestCode == 503 && resultCode == Result.Ok)
                {
                    ...
                    else
                    {
                        ...
                        {
                            var type = Methods.AttachmentFiles.Check_FileExtension(IntentController.CurrentPhotoPath);
                            if (type == "Image")
                            {
                                var unixTimestamp = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
                                string time2 = unixTimestamp.ToString(CultureInfo.InvariantCulture);
                                ...
                                await MessageController.SendMessageTask(this, Userid, "", "", IntentController.CurrentPhotoPath, time2, UserInfoData);
                                CanDoChat();
                                EmojisIconEditTextView.Text = "";
                            }
                        }
        ...
        public void OnSelection(MaterialDialog dialog, View itemView, int position, string itemString)
        {
            try
            {
                ...
                else if (itemString == GetString(Resource.String.Lbl_ImageGallery))
                {
                    ImageType = "Image";
                    // Check if we're running on Android 5.0 or higher 
                    if ((int)Build.VERSION.SdkInt < 23)
                ...
                else if (itemString == GetString(Resource.String.Lbl_TakeImageFromCamera))
                {
                    ImageType = "Camera";
                    // Check if we're running on Android 5.0 or higher 
                    if ((int)Build.VERSION.SdkInt < 23)
