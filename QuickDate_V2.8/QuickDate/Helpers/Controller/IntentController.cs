...
namespace QuickDate.Helpers.Controller
{
    public class IntentController
    {
        ...
        public void OpenIntentImageGallery(string title, bool allowMultiple, bool imageCropping = false)
        {
            try
            {
                ...
                if (imageCropping && Build.Manufacturer!.ToLower() == "samsung")
                {
                    ...
                    var myUri = Uri.FromFile(new File(Methods.Path.FolderDcimImage, DateTime.Now.ToShortDateString() + ".jpg"));
                    intent.PutExtra(MediaStore.ExtraOutput, myUri);
                    //intent.PutExtra("outputFormat", Bitmap.CompressFormat.jpg.ToString());
                }
