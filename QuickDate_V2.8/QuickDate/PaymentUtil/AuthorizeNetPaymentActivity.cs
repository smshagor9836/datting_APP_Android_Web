            ...
            try
            {
                if (MultilineWidget.Card.ValidateCard() && !string.IsNullOrEmpty(EtName.Text))
                {
                    ...
                    {                
                        ...
                        var (apiStatus, respond) = await RequestsAsync.Payments.TopWalletAuthorizeNetAsync(PayType, card.Number, card.ExpMonth.ToString(), card.ExpYear.ToString(), card.CVC, Price);
                        switch (apiStatus)
                        {
                            case 200:
