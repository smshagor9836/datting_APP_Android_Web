...
Class Profile extends Aj {
    public function add_invitation_links()
    {
        ...
        if (!empty($_POST['user_id']) && is_numeric($_POST['user_id']) && $_POST['user_id'] > 0 && (self::ActiveUser()->admin > 0 || self::ActiveUser()->id == $_POST['user_id']) && IfCanGenerateLink($_POST['user_id'])) {
    ...
    public function add_new_story(){
        ...
        if( $is_user_exist['id'] !== null && $is_user_exist['id'] > 0 ){
            ...
            if( !empty($exist['id']) && $exist['id'] !== null && $exist['id'] > 0 ){
                return array(
                    'status' => 400,
    ...
    public function private_avater() {
        ...
        if ($error == '') {
            ...
            if ($id[0]['id'] > 0) {
                ...
                $full_file_link = GetMedia($id[0]['file']);
                if (strpos($full_file_link, 'amazonaws') && !file_exists($id[0]['file'])) {
                    $getImage = fetchDataFromURL($full_file_link);
                    $importImage = file_put_contents($id[0]['file'], $getImage);
                }
        ...
        if($error === false){
            ...
            if (self::Config()->ffmpeg_sys == '1'){
                ...
                if (move_uploaded_file($_FILES['video_thumbnail']['tmp_name'], $filename)) {
                    ...
                    $org_file2 = 'upload' . $_DS . 'photos' . $_DS . date('Y') . $_DS . date('m') . $_DS . $key . '.' . $ext;
                    if (is_file($org_file)) {
                        if (self::Config()->watermark_system == 1) {
                    ...
                    if (is_file($org_file2)) {
                        if (self::Config()->watermark_system == 1) {
                            watermark_image($org_file2);
                        }
                        $upload_s3 = UploadToS3($org_file2, array(
                            'amazon' => 0
                        ));
                    }
...
//            }
            ...
            if ($error == '') {
                ...
                if (self::Config()->two_factor_type == 'both' || self::Config()->two_factor_type == 'email') {
                    $send_message_data       = array(
                        'from_email' => self::Config()->siteEmail,
    ...
    public function get_notifications() {
        global $_BASEPATH,$_DS,$config,$db;
        $data          = array(
            'status' => 200
        );
        ...
        if (!empty(self::ActiveUser()->coinpayments_txn_id)) {
            $result = coinpayments_api_call(array('key' => $config->coinpayments_public_key,
                                                  ...
                                                  'cmd' => 'get_tx_info',
                                                  'full' => '1',
                                                  'txid' => self::ActiveUser()->coinpayments_txn_id));
            if (!empty($result) && $result['status'] == 200) {
                if ($result['data']['status'] == -1) {
                    $db->where('id',self::ActiveUser()->id)->update('users',array('coinpayments_txn_id' => ''));
                    if (strpos($result['data']['checkout']['custom'], 'pro_') !== false) {
                        $notifications->createNotification(self::ActiveUser()->web_device_id, self::ActiveUser()->id, self::ActiveUser()->id, 'coinpayments_canceled', __('coinpayments_canceled'), '/pro');
                    }
                    else{
                        $notifications->createNotification(self::ActiveUser()->web_device_id, self::ActiveUser()->id, self::ActiveUser()->id, 'coinpayments_canceled', __('coinpayments_canceled'), '/credit');
                    }
                }
                elseif ($result['data']['status'] == 100) {
                    if (strpos($result['data']['checkout']['custom'], 'pro_') !== false) {
                        $pro = explode('_', $result['data']['checkout']['custom']);
                        if (!empty($pro) && !empty($pro[1]) && in_array($pro[1], array(1,2,3,4))) {
                            $price = $result['data']['checkout']['amountf'];
                            $response[ 'location' ] = '/ProSuccess?mode=pro';
                            $protime                = time();
                            $is_pro                 = "1";
                            $pro_type               = Secure($pro[1]);
                            $updated                = $db->where('id', self::ActiveUser()->id)->update('users', array(
                                'pro_time' => $protime,
                                'is_pro' => $is_pro,
                                'pro_type' => $pro_type
                            ...
                            $notifications->createNotification(self::ActiveUser()->web_device_id, self::ActiveUser()->id, self::ActiveUser()->id, 'coinpayments_approved', __('coinpayments_approved'), '/pro');
                        }
                    }
                    else{
                        $db->where('id',self::ActiveUser()->id)->update('users',array('coinpayments_txn_id' => '',
                        ...
                        $notifications->createNotification(self::ActiveUser()->web_device_id, self::ActiveUser()->id, self::ActiveUser()->id, 'coinpayments_approved', __('coinpayments_approved'), '/credit');
                    }
                }
            }
        }
    ...
    public function request_payment() {
        ...
        if (Wo_IsUserPaymentRequested(self::ActiveUser()->id) === true) {
            $error = __('You have already a pending request.');
            ...
            $error = __('Invalid amount value');
        } else if ((self::ActiveUser()->aff_balance < $_POST['amount'])) {
            $error = __('Invalid amount value, your amount is:') . ' ' . self::Config()->currency_symbol . number_format(self::ActiveUser()->aff_balance, 2);
        } else if (self::Config()->m_withdrawal > $_POST['amount']) {
            $error = __('Invalid amount value, minimum withdrawal request is:') . ' '. self::Config()->currency_symbol . self::Config()->m_withdrawal;
        }
        if (empty($error)) {
            ...
            if (!empty($_POST['withdraw_method']) && $_POST['withdraw_method'] == 'bank' && !empty($_POST['iban']) && !empty($_POST['country']) && !empty($_POST['full_name']) && !empty($_POST['swift_code']) && !empty($_POST['address'])) {
                ...
                $insert_array['user_id'] = self::ActiveUser()->id;
                $insert_array['time'] = time();
                $insert_array['amount'] = $amount;
                $insert_array['full_amount'] = self::ActiveUser()->aff_balance;
    ...
    public function my_info()
    {
        ...
        if (!empty($_POST['mediafiles']) || !empty($_POST['my_information']) || !empty($_POST['friends']) || !empty($_POST['liked_users']) || !empty($_POST['disliked_users'])) {
            ...
            if (!empty(self::ActiveUser()->info_file)) {
                unlink(self::ActiveUser()->info_file);
            }
            $user_info = array();
            $html = '';
            if (!empty($_POST['my_information'])) {
                $user_info['setting'] = self::ActiveUser();
                $user_info['setting']->session = $db->where('user_id', self::ActiveUser()->id)->orderBy('time', 'DESC')->get('sessions', null, array('*'));
                $user_info['setting']->block = array();
                $blocks = $db->where('user_id', self::ActiveUser()->id)->orderBy('id', 'DESC')->get('blocks', null, array('*'));
                if (!empty($blocks)) {
            ...
            if (!empty($_POST['mediafiles'])) {
                ...
                $user_info['mediafiles'] = self::ActiveUser()->mediafiles;
            }
            $file_path          = $_BASEPATH . 'themes' . $_DS . self::Config()->theme . $_DS.'user_info.php';
            ob_start();
            require($file_path);
            $html = ob_get_contents();
            ob_end_clean();
            ...
            fwrite($file, $html);
            fclose($file);
            $db->where('id',self::ActiveUser()->id)->update('users',array('info_file' => $hash));
            return array(
                'status' => 200
            );
    ...
    public function download_user_info()
    {
        ...
        if (!empty(self::ActiveUser()->info_file)) {
            $file = self::ActiveUser()->info_file;
           $filepath = $file; // upload/files/2019/20/adsoasdhalsdkjalsdjalksd.html
            ...
            if(file_exists($filepath)) {
               ...
               header('Content-Disposition: attachment; filename="'.self::ActiveUser()->username.'.html"');
               header('Expires: 0');
               ...
               unlink($filepath);
               // remove user data
               $db->where('id',self::ActiveUser()->id)->update('users',array('info_file' => ''));
               exit;
            }
        }
    }
    public function get_instagram_posts()
    {
        ...
        if (empty($_COOKIE['instagram_access_token']) || empty($_COOKIE['instagram_user_id'])) {
            return array(
                'status' => 403,
                'message' => __('token_expired')
            );
        }
        $instagram_after = '';
        if (!empty($_POST['instagram_after'])) {
            $instagram_after = Secure($_POST['instagram_after']);
        }

        $posts = GetInstagramPosts(array('instagram_access_token' => Secure($_COOKIE['instagram_access_token']),
                                         'instagram_user_id' => Secure($_COOKIE['instagram_user_id']),
                                         'instagram_after' => secure($instagram_after)));
        if (!empty($posts)) {
            if ($posts['status'] == 200 && !empty($posts['posts']) && !empty($posts['posts']['data'])) {
                $html = '';
                $file_path          = $_BASEPATH . 'themes' . $_DS . self::Config()->theme . $_DS.'partails' . $_DS.'instagram' . $_DS. 'instagram_posts.php';
                $next = false;
                $instagram_after = '';
                $instagram_ids = array();
                if (!empty($_COOKIE['instagram_ids'])) {
                    $instagram_ids = json_decode($_COOKIE['instagram_ids'],true);
                }
                foreach ($posts['posts']['data'] as $key => $post) {
                    if ($post['media_type'] == 'VIDEO' || $post['media_type'] == 'IMAGE' || $post['media_type'] == 'CAROUSEL_ALBUM') {
                        if (file_exists($file_path)) {
                            $post['media_image'] = $post['media_url'];
                            $post['type_text'] = $post['media_url'];
                            if ($post['media_type'] == 'VIDEO' && !empty($post['thumbnail_url'])) {
                                $post['media_image'] = $post['thumbnail_url'];
                            }
                            $post['imported'] = $db->where('user_id',self::ActiveUser()->id)->where('instagram_post_id',$post['id'])->getValue('mediafiles','COUNT(*)');
                            if (!in_array($post['id'],$instagram_ids)) {
                                $instagram_ids[] = Secure($post['id']);
                            }
                            ob_start();
                            require($file_path);
                            $html .= ob_get_contents();
                ...
                if (!empty($instagram_ids)) {
                    setcookie("instagram_ids", json_encode($instagram_ids), time() + (24 * 60 * 60), '/');
                }
                if (!empty($posts['posts']['paging']) && !empty($posts['posts']['paging']['next']) && !empty($posts['posts']['paging']['cursors']) && !empty($posts['posts']['paging']['cursors']['after'])) {
                    $instagram_after = Secure($posts['posts']['paging']['cursors']['after']);
                    $next = true;
                }
                return array(
                    'status' => 200,
                    'html' => $html,
                    'next' => $next,
                    'instagram_after' => $instagram_after
                ); 
            }
            elseif ($posts['status'] == 400 && !empty($posts['message'])) {
                if (!empty($_COOKIE['instagram_access_token'])) {
                    unset($_COOKIE['instagram_access_token']);
                }
                if (!empty($_COOKIE['instagram_user_id'])) {
                    unset($_COOKIE['instagram_user_id']);
                }
                setcookie('instagram_access_token', '', 1, '/');
                setcookie('instagram_user_id', '', 1, '/');
                return array(
                    'status' => 400,
    ...
    public function import_instagram_post()
    {
        global $_BASEPATH, $_DS,$db,$site_url,$config,$_AJAX;
        if (self::ActiveUser() == NULL) {
            return array(
        ...
        if (empty($_POST['id'])) {
            return array(
                'status' => 403,
                'message' => __('missing__id__parameter.')
            );
        }
        $post_data =     GetInstagramPostById(array('instagram_access_token' => Secure($_COOKIE['instagram_access_token']),
                                                    'instagram_user_id' => Secure($_COOKIE['instagram_user_id']),
                                                    'id' => secure($_POST['id'])));
        if (!empty($post_data)) {
            if ($post_data['status'] == 200) {
                ...
                $media['user_id'] = self::ActiveUser()->id;
                $media['is_confirmed'] = '1';
                $media['instagram_post_id'] = $post_data['id'];
                $media['created_at'] = date('Y-m-d H:i:s');
                if ($post_data['media_type'] == 'VIDEO') {
                    $media['file'] = $post_data['thumbnail_url'];
                    $media['is_video'] = '1';
                    $media['video_file'] = $post_data['file_url'];
                    $saved = $db->insert('mediafiles', $media);
                }
                elseif($post_data['media_type'] == 'IMAGE'){
                    $media['file'] = $post_data['file_url'];
                    $saved = $db->insert('mediafiles', $media);
                }
                elseif($post_data['media_type'] == 'CAROUSEL_ALBUM' && !empty($post_data['imported'])){
                    foreach ($post_data['imported'] as $key => $value) {
                        $media = array();
                        $media['user_id'] = self::ActiveUser()->id;
                        $media['is_confirmed'] = '1';
                        $media['instagram_post_id'] = $post_data['id'];
                        ...
                        if ($value['media_type'] == 'VIDEO') {
                            $media['file'] = $value['thumbnail_url'];
                            $media['is_video'] = '1';
                            $media['video_file'] = $value['file_url'];
                            $saved = $db->insert('mediafiles', $media);
                        }
                        elseif($value['media_type'] == 'IMAGE'){
                            $media['file'] = $value['file_url'];
                            $saved = $db->insert('mediafiles', $media);
                        }
                    }
                }
    ...
    public function import_instagram_posts()
    {
        ...
        if (empty($_POST['ids']) || empty($_COOKIE['instagram_ids'])) {
            return array(
                'status' => 403,
                'message' => __('missing__id__parameter.')
            );
        }
        $instagram_ids = $_POST['ids'];
        $instagram_saved_ids = json_decode($_COOKIE['instagram_ids'],true);
        $instagram_after = '';
        $imported_ids = array();
        $array_intersect = $sub = array_intersect($instagram_ids,$instagram_saved_ids);
        if (!empty($array_intersect) && !empty($instagram_saved_ids)) {
            $data = array(
                'status' => 200,
                'message' => __('check_after_some'),
            );
            ob_end_clean();
            ...
            foreach ($instagram_saved_ids as $key2 => $value2) {
                if (!empty($sub)) {
                    $posts = GetInstagramPosts(array('instagram_access_token' => Secure($_COOKIE['instagram_access_token']),
                                                     'instagram_user_id' => Secure($_COOKIE['instagram_user_id']),
                                                     'instagram_after' => $instagram_after));
                    if (!empty($posts)) {
                        if ($posts['status'] == 200 && !empty($posts['posts']) && !empty($posts['posts']['data'])) {
                            ...
                            foreach ($posts['posts']['data'] as $key3 => $post) {
                                if (in_array($post['id'], $array_intersect)) {

                                    if ($post['media_type'] == 'VIDEO' || $post['media_type'] == 'IMAGE' || $post['media_type'] == 'CAROUSEL_ALBUM') {
                                        $imported_ids[] = $post['id'];
                                        if (($key2 = array_search($post['id'], $sub)) !== false) {
                                            unset($sub[$key2]);
                                        }
                                        if ($post['media_type'] == 'VIDEO' && !empty($post['thumbnail_url'])) {
                                            $file_url = ImportFileFromUrl($post['media_url'],$post['media_type']);
                                            if (!empty($file_url)) {
                                                $thumbnail_url = ImportFileFromUrl($post['thumbnail_url'],'IMAGE');
                                                if (!empty($thumbnail_url)) {
                                                    $media = array();
                                                    $media['user_id'] = self::ActiveUser()->id;
                                                    $media['is_confirmed'] = '1';
                                                    $media['instagram_post_id'] = $post['id'];
                                                    $media['created_at'] = date('Y-m-d H:i:s');
                                                    $media['file'] = $thumbnail_url;
                                                    $media['is_video'] = '1';
                                                    $media['video_file'] = $file_url;
                                                    $saved = $db->insert('mediafiles', $media);
                                                }
                                            }
                                        }
                                        ...
                                        elseif($post['media_type'] == 'CAROUSEL_ALBUM' && !empty($post['children']) && !empty($post['children']['data'])){
                                            foreach ($post['children']['data'] as $key => $value) {
